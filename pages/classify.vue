                ...
                <view class="tn-classify__content__sub-classify">
                  ...
                  <view class="tn-classify__content__sub-classify__content tn-flex tn-flex-wrap tn-flex-col-center tn-flex-row-left">
                    <view
                      ...
                      :key="index"
                      class="tn-classify__content__sub-classify__content__item tn-flex tn-flex-wrap tn-flex-row-center item"  @click="tn('/pages/details?id='+item._id)"
                    >
                      ...
                      <view class="tn-classify__content__sub-classify__content__title tn-margin-bottom-sm itemText">{{ item.title }}</view>
                    </view>
                  </view>
                </view>
              </view>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        classifyContent: {
          // 推荐商品
          recommendProduct: [
            {image: 'https://env-00jx4xgopeln.normal.cloudstatic.cn/鲜虾菌菇豆腐汤 (2).jpg?expire_at=1711177635&er_sign=e11b89c46cdead7ec3651cc368888c61', title: '推荐商品1'},
            {image: 'https://www.foodiesfeed.com/wp-content/uploads/2023/03/blueberries-on-a-tree-macro-detail.jpg', title: '推荐商品2'},
            {image: 'https://env-00jx4xgopeln.normal.cloudstatic.cn/番茄粉丝.jpg?expire_at=1711178303&er_sign=d722904f282519b9e75dbd8349a2057a', title: '推荐商品3'}
          ],
          // 每个子栏目下的内容
          subClassify: [
            {
    ...
    computed: {
			currentList(){
				console.log(this.tabbar[this.currentTabbarIndex]);
				console.log(this.totalList[0]);
				let res = this.totalList.filter(item=>item.classify === this.tabbar[this.currentTabbarIndex]);
				return res;
			},
      tabbarItemClass() {
        return index => {
    ...
    onReady:async function() {
      this.$nextTick(() => {
        ...
        this.getTabbarItemRect()
			...
			let res =await db.field("id,swiperImgs,title,classify").get();
