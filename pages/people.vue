<template>
	<view class="template-business tn-safe-area-inset-bottom">
		...
		<view class="" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			<swiper class="card-swiper tn-margin-top-sm" :circular="true" :autoplay="true" duration="500" interval="5000"
				previous-margin="75rpx" next-margin="75rpx" @change="cardSwiper">
				<swiper-item v-for="(item,index) in swiperList" :key="index" :class="cardCur==index?'cur':''">
					<!-- <view class="swiper-item image-banner">
            <image :src="item.url" mode="aspectFill" v-if="item.type=='image'"></image>
          </view> -->

					<view class="swiper-item image-banner tn-shadow-blur"
	...
	export default {
			...
			return {
				...
				list1: [{
					...
					},
					{
						...
						title: '小美',
						content: '累计发布：41道菜肴',
						url: 'https://env-00jx4xgopeln.normal.cloudstatic.cn/avater.png?expire_at=1712395186&er_sign=35a275d82b0ccb4d3d28e56c0e130a03',
						color: 'yellowgreen'
					},
					{
						icon: 'light-fill',
		...
		async onLoad() {
			...
			const res = await db
				...
				.get();
				this.list1 = res.result.data.sort((a, b) => b.total - a.total).map(item => {
			  let res = {
			    icon: 'light-fill',
			    title: item.author,
			    content: `累计发布：${item.total}道菜肴`,
			    url: 'https://env-00jx4xgopeln.normal.cloudstatic.cn/avater.png?expire_at=1712395186&er_sign=35a275d82b0ccb4d3d28e56c0e130a03',
			    color: 'yellowgreen'
			  };
			  return res;
